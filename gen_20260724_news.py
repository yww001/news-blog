...
# 20 news items with image prompts
NEWS = [
    {
        "num": "01",
    ...
    },
    {
        ...
        "tag": "科技",
        "prompt": "Massive offshore wind farm in the ocean with dozens of wind turbines spinning,福建 coastal scenery, clear blue sea, clean energy, photorealistic, ultra detailed, 8K, high resolution, no text"
    },
]

def generate_image(prompt, num, retry=2):
        ...
        try:
            ...
            if "choices" in data and len(data["choices"]) > 0:
                ...
                if content.startswith("data:image/png;base64,"):
                    b64_data = content.replace("data:image/png;base64,", "")
    ...
    print(f"  FAILED after {retry} attempts: {num}")
    return False

def main():
    os.makedirs(IMG_DIR, exist_ok=True)
    
    print(f"Generating 20 images for {DATE_STR}...")
    results = {}
    
    for item in NEWS:
        num = item["num"]
        prompt = item["prompt"]
        print(f"\nGenerating image {num}: {item['title']}")
        ok = generate_image(prompt, num)
        results[num] = ok
    ...
    print(f"\nTotal: {success_count}/20 succeeded")
