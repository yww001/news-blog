...
# Find ALL article lines correctly
article_starts = []
article_ends = []
for i, line in enumerate(lines):
    if '<article class="news-card"' in line:
        article_starts.append(i)
    if '</article>' in line and article_starts and (not article_ends or i > article_ends[-1]):
        article_ends.append(i)

first_article_line = article_starts[0] if article_starts else None
last_article_end_line = article_ends[-1] if article_ends else None

print(f"First article at line: {first_article_line + 1 if first_article_line else None}")
print(f"Last article ends at line: {last_article_end_line + 1 if last_article_end_line else None}")
print(f"Total articles: {len(article_starts)}")

# Find news-grid opening line
news_grid_line = None
for i, line in enumerate(lines):
    if 'class="news-grid"' in line:
        news_grid_line = i
        break
print(f"News-grid at line: {news_grid_line + 1 if news_grid_line else None}")
...
# Keep lines 0 to first_article_line - 1 (before first article)
# Then add new articles
# Then add lines from last_article_end_line + 1 to end
before = lines[:first_article_line]
after = lines[last_article_end_line + 1:]
...
# Verify dates
date_count = verify.count(date_str)
print(f"Date '{date_str}' appears {date_count} times")