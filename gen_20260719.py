...
NEWS = [
    {
        "num": "01", "tag": "科技",
        "title": "上海世界人工智能大会发布星枢计划首批卫星数据",
        "summary": "2026世界人工智能大会期间，上海太空算力产业传来重大进展，星枢计划首批12颗试验卫星成功传回首批数据。该计划由复旦大学与上海星枢天算联合研制，旨在构建天地一体化算力网络。卫星搭载太赫兹通信载荷，理论传输速率可达5G的100倍，首批数据已应用于上海智慧城市基础设施建设，全球太空算力竞争进入新阶段。",
    ...
    },
    {
        ...
        "summary": "国家主席习近平在哈萨克斯坦首都阿斯塔纳出席中亚合作峰会并发表主旨演讲，宣布中国将建立一带一路人工智能国际合作机制，未来5年向发展中国家提供100亿美元AI发展基金。习近平表示，人工智能治理需要全球共同参与，中国愿与各国分享AI技术成果。这一倡议获得中亚五国和多个新兴市场国家的积极响应。",
    ...
    },
    {
        ...
        "summary": "国家统计局发布2026年上半年经济数据，上半年国内生产总值同比增长5.3%，其中二季度增长4.9%。数据显示，高技术制造业投资增长12.4%，新能源汽车、锂电池和太阳能电池出口增长强劲。统计局表示，下半年将进一步加大宏观政策调节力度，推动经济持续回升向好，实现全年5%左右的发展目标。",
    ...
    },
    {
        ...
        "summary": "百度在2026世界人工智能大会上正式发布文心大模型5.0版本，官方称其在多项基准测试中与OpenAI GPT-5持平，部分中文理解任务实现超越。文心5.0采用全新MoE架构，参数量突破万亿级，推理速度提升3倍。百度同时宣布向开发者免费开放API接口，并启动文心百川计划，目标是3年内赋能100万家中小企业AI升级。",
    ...
    {
        "num": "10", "tag": "国际",
        "title": "英国正式申请加入金砖国家 约翰逊称其为历史性决定",
        "summary": "英国外交大臣在喀山举行的金砖国家外长会议上正式递交加入申请，成为首个申请加入金砖机制的七国集团成员。英国首相约翰逊表示，英国脱欧后需要重新定位全球伙伴关系，金砖国家代表新兴市场和发展中国家的崛起，英国不能缺席。金砖扩员问题将在今年10月喀山金砖峰会上正式讨论。",
    ...
    },
    {
        ...
        "summary": "青岛海水稻研发中心宣布，中国科学家在新疆塔克拉玛干沙漠边缘的盐碱地上试种的耐盐碱水稻海稻86获得突破，亩产达到812公斤，刷新该类土地水稻种植纪录。海稻86可在pH 9.5以下的盐碱地生长，灌溉用水减少40%。袁隆平团队表示，按目前推广速度，2030年前可为中国新增1亿亩良田，有效保障国家粮食安全。",
    ...
    },
    {
        ...
        "summary": "印度空间研究组织宣布，月船4号探测器成功在月球背面着陆，印度成为全球第三个实现月背软着陆的国家。探测器携带月球车和钻探设备，将开展月背地质构造和水冰资源勘探。印度总理莫迪表示，这是印度航天事业的里程碑，证明发展中国家同样可以在深空探索领域取得突破。中国嫦娥系列为月背探测提供了重要技术参考。",
    ...
    },
    {
        ...
        "summary": "国产3A游戏《黑神话：悟空》首个DLC大闹天宫全球同步上线，上线首日销量突破500万份。游戏科学CEO冯骥表示，DLC历时2年开发，包含全新地图、Boss战和剧情内容。Steam平台同时在线玩家峰值突破180万，创造单机游戏新纪录。海外玩家评分高达9.2分，成为中国文化出海的现象级产品。",
...
def generate_image(news_num, prompt):
    filepath = os.path.join(IMAGES_DIR, f"news_{DATE_ID}_{news_num}.png")
    if os.path.exists(filepath):
        size = os.path.getsize(filepath)
        print(f"  [SKIP] news_{DATE_ID}_{news_num}.png 已存在 ({size} bytes)")
        return True
    ...
    for attempt in range(2):
        try:
            ...
            # cogview-3-flash 返回格式: [{"url": "..."}]
            if isinstance(content, list) and len(content) > 0:
                img_url = content[0].get("url") or content[0]
            else:
                print(f"  [ERROR] 未知响应格式: {str(content)[:200]}")
                continue
            ...
            print(f"  [OK] news_{DATE_ID}_{news_num}.png ({len(img_resp.content)} bytes)")
            return True
...
for i, news in enumerate(NEWS):
    print(f"  [{i+1}/20] news_{DATE_ID}_{news['num']}.png")
    if generate_image(news["num"], news["prompt"]):
        success_count += 1
print(f"  图片完成: {success_count}/20 成功")
...
print("\n[Step 3] Git 推送...")
import subprocess
repo = "/home/swg/.openclaw/workspace/news-blog"

subprocess.run(["git", "add", "index.html"], cwd=repo)
subprocess.run(["git", "add", f"images/news_{DATE_ID}_*.png"], cwd=repo)

result = subprocess.run(["git", "commit", "-m", "更新首页：2026-07-19"], cwd=repo, capture_output=True, text=True)
print(f"  commit: {result.stdout} {result.stderr}")

env = os.environ.copy()
env["GIT_SSH_COMMAND"] = "ssh -i ~/.ssh/id_ed25519"
result = subprocess.run(["git", "push", "origin", "main"], cwd=repo, capture_output=True, text=True, env=env)
